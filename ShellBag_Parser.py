	...
	for i in range(Sub_Key_Cut):
		try:
			...
			if not name in ("MRUListEx", "NodeSlot"):
				...
				loop_Sub_Key = OpenKey(Var_Hive, Path)
				ts = QueryInfoKey(loop_Sub_Key)[2]
				print(' '*cut +'{} {}'.format(Key_Path,Win_ts(ts)))
				if(QueryInfoKey(Var_Key)[0] != 3):
					cut+=1
					loop_Sub_Parser(Var_Key, Key_Path, QueryInfoKey(Var_Key)[0], cut)	
		except WindowsError:
			pass
...
for i in range(BagMRU_Cut):
	try:
		...
		if not name in ("MRUListEx", "NodeSlot", "NodeSlots"):
			...
			print(Sub_BagMRU_Path, name, Win_ts(ts))
			if(QueryInfoKey(Sub_Var_Key)[0] != 3):
				cut+=1
				loop_Sub_Parser(Sub_Var_Key, Sub_BagMRU_Path, QueryInfoKey(Sub_Var_Key)[0], cut)
